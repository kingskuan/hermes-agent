    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if webhook_url:
                ...
                # ── HTML Image Studio ──────────────────────────────────────
                # Inject a /studio route into the Tornado webhook app so the
                # web-based HTML Image Studio is reachable at <host>/studio
                try:
                    import tornado.web as _tw
                    import pathlib as _pl

                    _studio_path = _pl.Path("/opt/hermes/studio/index.html")

                    class _StudioHandler(_tw.RequestHandler):
                        def get(self):
                            if _studio_path.exists():
                                self.set_header("Content-Type", "text/html; charset=utf-8")
                                self.write(_studio_path.read_bytes())
                            else:
                                self.set_status(404)
                                self.write("Studio not found")

                    _tornado_app = self._app.updater._httpd.application
                    _tornado_app.add_handlers(r".*", [(r"/studio/?", _StudioHandler)])
                    logger.info("[%s] HTML Image Studio available at /studio", self.name)
                except Exception as _e:
                    logger.warning("[%s] Could not inject /studio route: %s", self.name, _e)
                # ── End HTML Image Studio ──────────────────────────────────
            else:
